...
def yolo_loss(y_true, y_pred):
    ...
    iou_scores = iou(predict_xy_min, predict_xy_max,
                     label_xy_min, label_xy_max)  # ? * 7 * 7 * 2 * 1
    ...
    box_loss = 5 * box_mask * response_mask * \
        K.square((label_xy - predict_xy) / 448)
    box_loss += 5 * box_mask * response_mask * \
